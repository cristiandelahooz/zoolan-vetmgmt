...
(
    ...
    notes         TEXT                                    NOT NULL,
    grooming_date TIMESTAMP WITHOUT TIME ZONE             NOT NULL,
    pet           BIGINT                                  NOT NULL,
...
CREATE TABLE IF NOT EXISTS grooming_sessions_log
(
    ...
    notes             TEXT,
    notes_mod         BOOLEAN,
    grooming_date     TIMESTAMP WITHOUT TIME ZONE,
...
(
    ...
    updated_at        TIMESTAMP WITHOUT TIME ZONE,
    updated_at_mod    BOOLEAN,
    CONSTRAINT pk_offerings_log PRIMARY KEY (rev, id)
);
...
(
    ...
    CONSTRAINT pk_invoice_offerings PRIMARY KEY (id)
);
...
(
    ...
    amount       DECIMAL(10, 2),
    amount_mod   BOOLEAN,
    CONSTRAINT pk_invoice_offerings_log PRIMARY KEY (rev, id)
);
...
CREATE INDEX idx_invoice_offerings_invoice_id ON invoice_offerings (invoice);

CREATE INDEX idx_invoice_offerings_offering_id ON invoice_offerings (offering);

CREATE INDEX idx_offerings_category ON offerings (offering_type);

CREATE INDEX idx_offerings_active ON offerings (active);

CREATE INDEX idx_invoices_consultation ON invoices (consultation);

CREATE INDEX IF NOT EXISTS idx_gs_grooming_date ON grooming_sessions (grooming_date);
...
ALTER TABLE offerings_log
    ADD CONSTRAINT FK_OFFERINGS_log_ON_REV FOREIGN KEY (rev) REFERENCES revision (id);

ALTER TABLE invoice_offerings_log
    ADD CONSTRAINT FK_INVOICE_OFFERINGS_log_ON_REV FOREIGN KEY (rev) REFERENCES revision (id);

ALTER TABLE invoice_offerings
    ADD CONSTRAINT FK_INVOICE_OFFERINGS_ON_INVOICE FOREIGN KEY (invoice) REFERENCES invoices (code);

ALTER TABLE invoice_offerings
    ADD CONSTRAINT FK_INVOICE_OFFERINGS_ON_OFFERING FOREIGN KEY (offering) REFERENCES offerings (id);
...
ALTER TABLE grooming_sessions
    ADD CONSTRAINT fk_gs_pet
        FOREIGN KEY (pet) REFERENCES pets (id);

ALTER TABLE grooming_sessions
    ADD CONSTRAINT fk_gs_groomer
        FOREIGN KEY (groomer) REFERENCES employee (employee_id);
