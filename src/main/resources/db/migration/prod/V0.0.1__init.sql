...
(
    ...
    notes             TEXT         not null,
    prescription      TEXT,
    status            varchar(255) not null check (status in ('PENDIENTE', 'EN_PROCESO', 'COMPLETADO', 'CANCELADO')),
    treatment         TEXT,
    primary key (id)
);
...
(
    ...
    notes                 TEXT,
    prescription          TEXT,
    status                varchar(255) check (status in ('PENDIENTE', 'EN_PROCESO', 'COMPLETADO', 'CANCELADO')),
    treatment             TEXT,
    primary key (rev, id)
);
...
(
    ...
    grooming_mod            boolean,
    grooming_notes_mod      boolean,
    issued_date             date,
...
(
    ...
    status                  varchar(255) not null check (status in ('ESPERANDO', 'EN_PROCESO', 'COMPLETADO', 'CANCELADO')),
    type                    varchar(255) not null check (type in ('MEDICA', 'GROOMING')),
    primary key (id)
);
...
(
    ...
    reason_for_visit            varchar(255),
    status                      varchar(255) check (status in ('ESPERANDO', 'EN_PROCESO', 'COMPLETADO', 'CANCELADO')),
    type                        varchar(255) check (type in ('MEDICA', 'GROOMING')),
    primary key (rev, id)
);
...
alter table if exists consultations
    add constraint FKkb6yt5bv7xks4c40xifo70qo6
...
alter table if exists invoices
    add constraint FKslk1mcifgfnivnmaou1xguo2c
        foreign key (grooming)
            references grooming_sessions;
...
alter table if exists waiting_room
    add constraint FKi1figkp7tc8g6h9w05puxf861
        foreign key (assigned_groomer)
            references employee;

alter table if exists waiting_room
    add constraint FK5c5q07fue7da05cxcr625895q
        foreign key (assigned_veterinarian)
            references employee;
