...
export const config: ViewConfig = {
    menu: {title: 'Fusionar'}, title: 'Fusionar Mascotas Duplicadas',
}
...
export default function PetMergeView() {
    ...
    const handlePetSelection = (pet: Pet, role: 'keep' | 'remove') => {
        setSelectedPets((prev) => ({
            ...prev, [role === 'keep' ? 'keepPet' : 'removePet']: pet,
        }))
    }
    ...
    const canMerge = selectedPets.keepPet && selectedPets.removePet && selectedPets.keepPet.id !== selectedPets.removePet.id

    const OwnersList = ({owners}: { owners: any[] }) => (<div>
    ...
    const ActionButtons = ({pet}: { pet: Pet }) => {
        ...
        return (<HorizontalLayout theme="spacing-s">
                <Button
    ...
    return (<>
            <main className="w-full h-full flex flex-col box-border gap-s p-m">
                <VerticalLayout className="w-full">
                    ...
                    <p style={{color: '#666', marginBottom: '1rem'}}>
                        Busca mascotas por nombre para encontrar posibles
                        duplicados. Podrás mantener una y transferir todos los
                        dueños a ella.
                    </p>
                    ...
                    {searchResults.length > 0 && (
                        <VerticalLayout className="w-full">
                            ...
                            <Grid items={searchResults} className="w-full"
                                  style={{height: '400px'}}>
                                <GridColumn header="Nombre"
                                            renderer={({item}) =>
                                                <span>{item.name || ''}</span>}
                                            width="130px"/>
                                <GridColumn
                                    ...
                                    width="100px"
                                />
                                <GridColumn header="Raza" renderer={({item}) =>
                                ...
                                <GridColumn
                                    header="Dueños"
                                    renderer={({item}) => <OwnersList
                                        owners={item.owners || []}/>}
                                    width="200px"
                                />
                                <GridColumn header="Acciones"
                                            renderer={({item}) => <ActionButtons
                                                pet={item}/>} width="180px"/>
                            </Grid>

                            {selectedPets.keepPet && (<div
                                    className="mt-4 p-4 border-2 border-green-300 bg-green-50 rounded-lg">
                                    <strong style={{color: '#166534'}}>✓ Mascota
                                        a mantener:</strong>
                                    <div>
                            ...
                            {selectedPets.removePet && (<div
                                    className="mt-2 p-4 border-2 border-red-300 bg-red-50 rounded-lg">
                                    <strong style={{color: '#dc2626'}}>✗ Mascota
                                        a eliminar:</strong>
                                    <div>
            ...
            >
                <VerticalLayout style={{width: '450px', gap: '1rem'}}>
                    <p>
                        <strong>¿Estás segura de que quieres fusionar estas
                            mascotas?</strong>
                    </p>

                    {selectedPets.keepPet && (<div
                            className="p-4 border-2 border-green-300 bg-green-50 rounded-lg">
                            <strong style={{color: '#166534'}}>🐾 SE
                                MANTENDRÁ:</strong>
                            <div>
                            ...
                            <div>
                                <strong>Dueños
                                    actuales:</strong> {selectedPets.keepPet.owners?.length || 0}
                            </div>
                        </div>)}

                    {selectedPets.removePet && (<div
                            className="p-4 border-2 border-red-300 bg-red-50 rounded-lg">
                            <strong style={{color: '#dc2626'}}>🗑 SE
                                ELIMINARÁ:</strong>
                            <div>
                            ...
                            <div>
                                <strong>Dueños que se
                                    trasladarán:</strong> {selectedPets.removePet.owners?.length || 0}
                            </div>
                        </div>)}

                    <div
                        className="p-3 bg-blue-50 border border-blue-200 rounded">
                        <strong>📝 Resultado:</strong> La mascota
                        "{selectedPets.keepPet?.name || ''}" tendrá todos los
                        dueños de
                        ambas mascotas.
                    </div>

                    <HorizontalLayout theme="spacing" className="mt-4"
                                      style={{justifyContent: 'flex-end'}}>
                        <Button theme="secondary"
                                onClick={() => setDialogOpen(false)}>
                            Cancelar
                        </Button>
                        <Button theme="primary error" onClick={handleMerge}
                                disabled={loading}>
