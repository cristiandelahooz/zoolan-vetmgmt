...
import CreateConsultationDTOModel
    from '@/generated/com/wornux/dto/request/CreateConsultationRequestDtoModel'
...
import type CreateConsultationDTO
    from '@/generated/com/wornux/dto/request/CreateConsultationRequestDto'
...
const createSelectedPetFromGridItem = (item: any): SelectedPet => ({
    ...
    type: item.type,
    breed: item.breed,
    ownerName: item.owners && item.owners.length > 0 ? `${item.owners[0].firstName} ${item.owners[0].lastName}` : 'Sin propietario'
});
                ...
                {isConsultationHistoryLoading.value ? (<div>Cargando
                    ...
                    >
                        ...
                        {consultationGridColumnDefinitions.map((column, index) => (
                            <GridColumn key={index} path={column.path}
