  ...
  return (
    <Dialog opened={isOpen} onOpenedChanged={({ detail }) => !detail.value && handleDialogClose()}>
      <div style={{ padding: '1rem', width: '800px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <TextField label="Veterinario seleccionado" value={selectedVeterinarianDisplayValue} readonly />

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
          <Button theme="tertiary" onClick={handleDialogClose}>
            Cancelar
          </Button>
          <Button theme="primary" onClick={handleVeterinarianSelection} disabled={isSelectionConfirmationDisabled}>
            Seleccionar
          </Button>
        </div>
      </div>
