...
public record UserDetailsImpl(User user) implements UserDetails {
  ...
  public Collection<? extends GrantedAuthority> getAuthorities() {
    ...
    if (user.getSystemRole() != null) {
      ...
      log.debug(
          "Added system role for user {}: ROLE_{}",
          user.getUsername(),
    ...
    if (user instanceof Employee employee) {
      if (employee.getEmployeeRole() != null) {
        authorities.add(
            new SimpleGrantedAuthority("ROLE_EMP_" + employee.getEmployeeRole().name()));
        log.debug(
            "Added employee role for user {}: ROLE_EMP_{}",
            user.getUsername(),
  ...
  public String getPassword() {
    log.debug(
        "Password retrieved for user: {} - Hash length: {}",
        user.getUsername(),
