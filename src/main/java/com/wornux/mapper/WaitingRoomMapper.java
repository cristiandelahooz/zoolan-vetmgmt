...
public interface WaitingRoomMapper {
    ...
    WaitingRoom toEntity(WaitingRoomCreateRequestDto dto, @Context ClientRepository clientRepository, @Context PetRepository petRepository);

    default Client mapClient(Long clientId, @Context ClientRepository clientRepository) {
        return clientId == null ? null : clientRepository.findById(clientId)
    ...
    default Pet mapPet(Long petId, @Context PetRepository petRepository) {
        return petId == null ? null : petRepository.findById(petId)
