...
public interface ServiceRepository extends JpaRepository<Service, Long>, JpaSpecificationExecutor<Service> {
   ...
   */
  @Query("SELECT s FROM Service s WHERE LOWER(s.name) LIKE LOWER(CONCAT('%', :name, '%')) AND s.active = true")
  ...
  @Query("SELECT s FROM Service s WHERE s.active = true AND s.serviceCategory = :serviceType ORDER BY s.name ASC")
  List<Service> findActiveServicesByServiceCategory(@Param("serviceCategory") ServiceCategory serviceCategory);

  @Query("SELECT s FROM Service s WHERE s.active = true AND (s.name LIKE %:searchTerm% OR s.description LIKE %:searchTerm%) ORDER BY s.name ASC")
