...
/**
 * Helper class for pet breed validation logic. Centralizes the validation logic to avoid code
...
public final class PetBreedValidationHelper {
     ...
     *
     * @param petType
     *     the pet type to validate against
     * @param breed
     *     the breed to validate
     * @param context
    ...
    public static boolean isValidBreedForType(PetType petType, String breed, ConstraintValidatorContext context) {
        ...
        if (!isValid && context != null) {
            ...
            context.buildConstraintViolationWithTemplate(String.format("Breed '%s' is not valid for pet type '%s'",
                    breed, petType.name())).addPropertyNode("breed").addConstraintViolation();
        }
