...
public class InvoiceViewTmp extends VerticalLayout {
    ...
    private Component buildHeader() {
        ...
        status.getStyle().set("background", "#d1f7e2").set("color", "#1a7f37").set("padding", "4px 12px")
                .set("border-radius", "6px").set("font-weight", "bold");

        left.add(invoiceTitle, status);
        left.getStyle().set("display", "flex").set("align-items", "center").set("gap", "1em");

        HorizontalLayout actions = new HorizontalLayout();
        actions.add(new Button("Online Payments ● ON"), new Button("More actions"),
    ...
    private Component buildSendSection() {
        ...
        sendInfo.add(new Span("Last sent: with Wave on October 7, 2024."),
    ...
    private Component buildManagePaymentSection() {
        ...
        layout.add(new Span("Amount due: $0.00 — Status: Your invoice is paid in full"),
