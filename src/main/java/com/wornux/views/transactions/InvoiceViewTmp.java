...
public class InvoiceViewTmp extends VerticalLayout {
    ...
    public InvoiceViewTmp() {
        ...
        add(buildHeader(), buildCreateSection(), buildSendSection(), buildTermsSection(), buildManagePaymentSection());
    }
    ...
    private Component buildHeader() {
        ...
        status.getStyle().set("background", "#d1f7e2").set("color", "#1a7f37").set("padding", "4px 12px").set(
                "border-radius", "6px").set("font-weight", "bold");

        left.add(invoiceTitle, status);
        left.getStyle().set("display", "flex").set("align-items", "center").set("gap", "1em");

        HorizontalLayout actions = new HorizontalLayout();
        actions.add(new Button("Online Payments ● ON"), new Button("More actions"), new Button(
    ...
    private Component buildCreateSection() {
        Details details = new Details("Create", new Span("Created on: September 25, 2024 at 9:14 PM AST"));
    ...
    private Component buildSendSection() {
        ...
        sendInfo.add(new Span("Last sent: with Wave on October 7, 2024."), new Span(
                "Last viewed by customer: on October 11, 2024 at 7:30 PM AST"));
    ...
    private Component buildTermsSection() {
        ...
        layout.setWidthFull();
        Span label = new Span("Require customer to agree to terms of service before paying by credit card");
        label.getStyle().set("color", "#888");

        Span toggle = new Span("OFF");
        toggle.getStyle().set("background", "#eee").set("padding", "4px 10px").set("border-radius", "12px");
    ...
    private Component buildManagePaymentSection() {
        ...
        layout.add(new Span("Amount due: $0.00 — Status: Your invoice is paid in full"), new Span(
                "October 10, 2024 - A payment for $337.50 was made using AMEX •••• 1014."));

        HorizontalLayout links = new HorizontalLayout();
        links.add(new Anchor("#", "Send a receipt"), new Anchor("#", "Edit payment"), new Anchor("#", "View details"));
