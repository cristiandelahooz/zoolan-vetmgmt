...
public class AppointmentsCalendarView extends VerticalLayout {
    ...
    private void configureCalendar() {
        ...
        calendar.setBusinessHours(
            new BusinessHours(LocalTime.of(8, 0), LocalTime.of(20, 0),
                BusinessHours.DEFAULT_BUSINESS_WEEK),
            new BusinessHours(LocalTime.of(9, 0), LocalTime.of(14, 0), 
                DayOfWeek.SATURDAY)
        );
    ...
    private JsonObject createDefaultInitialOptions() {
        ...
        initialOptions.put("eventTimeFormat", Json.parse("{\"hour\": \"2-digit\", \"minute\": \"2-digit\", \"meridiem\": false}"));
    ...
    private void loadAppointments() {
        try {
            ...
            List<Entry> entries = appointments.stream()
    ...
    private Entry convertToEntry(AppointmentResponseDto appointment) {
        ...
        entry.setDescription(String.format("Mascota: %s | Servicio: %s", 
                appointment.getPetName(), appointment.getServiceType()));
    ...
    private void openNewAppointmentDialog(LocalDateTime start, LocalDateTime end) {
        ...
        petSelect.setItemLabelGenerator(pet -> pet.getName() + " - " + 
                (pet.getOwners().isEmpty() ? "Sin dueño" : 
                pet.getOwners().get(0).getFirstName() + " " + pet.getOwners().get(0).getLastName()));
        
        // Load pets
        List<Pet> pets = petService.getAllPets(PageRequest.of(0, 1000))
                .stream()
                .map(dto -> {
                    Pet pet = new Pet();
                    pet.setId(dto.id());
