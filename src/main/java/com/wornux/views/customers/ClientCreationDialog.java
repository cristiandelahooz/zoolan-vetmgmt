...
public class ClientCreationDialog extends Dialog {
    ...
    private Button createClientTypeButton(String title, String description, VaadinIcon icon, Runnable action) {
        ...
        button.getStyle().set("min-height", "120px").set("border", "2px solid var(--lumo-contrast-20pct)").set(
                "border-radius", "var(--lumo-border-radius-l)").set("cursor", "pointer");

        // Hover effects
        button.getElement().addEventListener("mouseenter", e -> button.getStyle().set("border-color",
                "var(--lumo-primary-color)"));
        button.getElement().addEventListener("mouseleave", e -> button.getStyle().set("border-color",
