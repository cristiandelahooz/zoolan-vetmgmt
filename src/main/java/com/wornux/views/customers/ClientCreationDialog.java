...
public class ClientCreationDialog extends Dialog {
  ...
  private void setupDialog() {
    ...
    Button individualButton =
        createClientTypeButton(
            "Cliente Individual",
            "Persona física con cédula o pasaporte",
            VaadinIcon.USER,
    ...
    Button companyButton =
        createClientTypeButton(
            "Empresa", "Empresa o negocio con RNC", VaadinIcon.BUILDING, this::openCompanyForm);

    VerticalLayout buttonLayout = new VerticalLayout(individualButton, companyButton);
    buttonLayout.addClassNames(
        LumoUtility.JustifyContent.CENTER, LumoUtility.Gap.SMALL, LumoUtility.Width.FULL);
  ...
  private Button createClientTypeButton(
      String title, String description, VaadinIcon icon, Runnable action) {
    Div iconDiv = new Div(icon.create());
    iconDiv.addClassNames(
    ...
    buttonDescription.addClassNames(
    ...
    buttonContent.addClassNames(
        LumoUtility.AlignItems.CENTER, LumoUtility.Padding.LARGE, LumoUtility.Gap.SMALL);
    ...
    button
        .getElement()
        .addEventListener(
            "mouseenter", e -> button.getStyle().set("border-color", "var(--lumo-primary-color)"));
    button
        .getElement()
        .addEventListener(
            "mouseleave", e -> button.getStyle().set("border-color", "var(--lumo-contrast-20pct)"));
   ...
   *
   * @param callback Consumer that receives the created client
   */
