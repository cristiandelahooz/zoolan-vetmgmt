...
public class ClientForm extends Div {
    ...
    public ClientForm(ClientService service) {
        ...
        Arrays.asList(address, notes).forEach(c -> {
            c.setMinRows(4);
            c.setMaxRows(4);
        });
        ...
        sidebar.createContent(new H4("General information"), name, type, email, phone, address, hourlyRateField,
                contactPersonInformation, contactName, contactEmail, new Hr(), notes);
    ...
    private void archive(ClickEvent<Button> buttonClickEvent) {
        ...
        ConfirmDialog dialog = new ConfirmationDialogBuilder().withHeader("Archive Client Confirmation").withText(
                "Would you like to archive and disable <<%s>>?".formatted(element.getFirstName())).withCancelText("No")
                .withConfirmText("Yes, archive").withIcon(VaadinIcon.QUESTION_CIRCLE_O).onConfirm(event -> {
