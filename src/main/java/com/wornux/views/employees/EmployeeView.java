...
public class EmployeeView extends Div {
  ...
  public EmployeeView(@Qualifier("employeeServiceImpl") EmployeeService employeeService) {
    ...
    employeeForm.addEmployeeSavedListener(
        event -> {
          refreshAll();
        });
  ...
  private void createGrid(EmployeeService service, Specification<Employee> specification) {
    ...
    GridUtils.addColumn(
        grid,
        employee -> employee.getGender() != null ? employee.getGender().name() : "",
        "Género",
        "gender");
    ...
    GridUtils.addColumn(
            grid, employee -> employee.isActive() ? "Activo" : "Inactivo", "Estado", "active")
  ...
  private void refreshAll() {
    ...
    long count =
        employeeService
            .getAllEmployees(org.springframework.data.domain.Pageable.unpaged())
            .getTotalElements();
