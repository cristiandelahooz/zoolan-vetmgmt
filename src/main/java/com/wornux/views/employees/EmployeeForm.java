...
public class EmployeeForm extends Dialog {
  ...
  private final Binder<EmployeeUpdateRequestDto> binderUpdate =
      new Binder<>(EmployeeUpdateRequestDto.class);
  ...
  private void setupCreateBinder() {
    ...
    binder
        ...
        .withValidator(
            new StringLengthValidator(
                "La contraseña debe tener al menos 8 caracteres", MIN_PASSWORD_LENGTH, null))
    ...
    binder
        ...
        .withValidator(
            new RegexpValidator(
                "Proporcione un número de teléfono válido", DOMINICAN_PHONE_PATTERN, true))
    ...
    binder
        .forField(emergencyContactPhone)
  ...
  private void setupUpdateBinder() {
    ...
    binderUpdate
        .forField(emergencyContactName)
        .bind(
            EmployeeUpdateRequestDto::getEmergencyContactName,
            EmployeeUpdateRequestDto::setEmergencyContactName);

    binderUpdate
        .forField(emergencyContactPhone)
        .bind(
            EmployeeUpdateRequestDto::getEmergencyContactPhone,
  ...
  private void saveNew() {
    ...
    try {
      ...
      log.debug(
          "Creating employee with username: '{}', firstName: '{}', lastName: '{}'",
  ...
  private void configureFieldValidation() {
    ...
    emergencyContactPhone.addValueChangeListener(
        event -> validateEmergencyPhoneFormat(event.getValue()));
  ...
  private void validateEmergencyPhoneFormat(String phoneValue) {
    if (phoneValue != null && !phoneValue.trim().isEmpty()) {
      if (!phoneValue.matches(DOMINICAN_PHONE_PATTERN)) {
        ...
        emergencyContactPhone.setErrorMessage(
            "Formato de teléfono inválido (809, 849 o 829 + 7 dígitos)");
      } else {
  ...
  private HorizontalLayout createScheduleTemplateButtons() {
    ...
    HorizontalLayout templateLayout =
        new HorizontalLayout(mondayToFriday, fullTime, partTime, clearAll);
  ...
  private EmployeeUpdateRequestDto createUpdateDtoFromEmployee(Employee employee) {
    ...
    if (employee.getWorkScheduleDays() != null) {
      scheduleDtos =
          employee.getWorkScheduleDays().stream()
              .map(
                  day ->
                      WorkScheduleDayDto.builder()
  ...
  private void loadEmployeeSchedule(Employee employee) {
    ...
    if (employee.getWorkScheduleDays() != null && !employee.getWorkScheduleDays().isEmpty()) {
      List<WorkScheduleDayDto> scheduleDtos =
          employee.getWorkScheduleDays().stream()
              .map(
                  day ->
                      WorkScheduleDayDto.builder()
  ...
  private void initializeDefaultSchedule() {
    ...
    for (DayOfWeek day : DayOfWeek.values()) {
      WorkScheduleDayDto scheduleDay =
          WorkScheduleDayDto.builder()
              .dayOfWeek(day)
              .startTime(null)
              .endTime(null)
    ...
    scheduleGrid
        .addComponentColumn(
            day -> {
              ...
              offDayCheckbox.addValueChangeListener(
                  e -> {
