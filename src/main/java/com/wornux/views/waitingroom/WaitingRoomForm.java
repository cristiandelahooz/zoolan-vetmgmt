...
public class WaitingRoomForm extends Dialog {
    ...
    public WaitingRoomForm(WaitingRoomService waitingRoomService, ClientService clientService, PetService petService) {
        ...
        // ===== Layout
        FormLayout formLayout = new FormLayout(clientField, // Cliente (readonly) + botón suffix
                petField, // Mascota (se habilita con el cliente)
                reasonField, priorityField, notesField, arrivalTimeField);
    ...
    private void save() {
        ...
        LocalDateTime arrival = (editingWaitingRoom == null) ? now : editingWaitingRoom.getArrivalTime();

        WaitingRoomCreateRequestDto dto = WaitingRoomCreateRequestDto.builder().clientId(selectedClient.getId())
                .petId(petField.getValue().getId()).reasonForVisit(reasonField.getValue())
                .priority(priorityField.getValue()).notes(notesField.getValue()).arrivalTime(arrival).build();
