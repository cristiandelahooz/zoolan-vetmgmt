...
public class IndividualClientView extends Div {
    ...
    private final MultiSelectComboBox<ClientRating> rating = new MultiSelectComboBox<>("Calificación");
    ...
    public IndividualClientView(@Qualifier("clientServiceImpl") ClientService clientService) {
        ...
        individualClientForm.addClientSavedListener(event -> {
            refreshAll();
    ...
    private void createGrid(ClientService service, Specification<Client> specification) {
        ...
        GridUtils.addColumn(grid, client -> client.getFirstName() + " " + client.getLastName(), "Nombre Completo",
                "firstName", "lastName");
        ...
        GridUtils.addColumn(grid, client -> client.getGender() != null ? client.getGender().name() : "", "Género",
                "gender");
    ...
    public Specification<Client> createFilterSpecification() {
        return (root, query, builder) -> {
            ...
            Predicate individualClientPredicate = builder.or(builder.isNotNull(root.get("cedula")),
                    builder.isNotNull(root.get("passport")));
            ...
            return builder.and(individualClientPredicate, searchPredicate, ratingPredicate, genderPredicate);
        };
    }
    ...
    private Predicate createSearchPredicate(Root<Client> root, CriteriaBuilder builder) {
        return predicateForTextField(root, builder,
                new String[] { "firstName", "lastName", "email", "cedula", "passport" }, searchField.getValue());
    }

    private Predicate createRatingPredicate(Root<Client> root, CriteriaBuilder builder) {
        return createPredicateForSelectedItems(Optional.ofNullable(rating.getSelectedItems()),
                items -> root.get("rating").in(items), builder);
    }
    ...
    private void refreshAll() {
        ...
        long count = clientService.getAllActiveClients().stream()
                .filter(client -> client.getCedula() != null || client.getPassport() != null).count();
    ...
    private Component createFilter() {
        ...
        Set.of(rating, gender).forEach(c -> {
            c.setWidthFull();
    ...
    private Div createTitle() {
        ...
        Icon icon = InfoIcon.INFO_CIRCLE.create("Gestionar clientes individuales con cédula o pasaporte.");
