...
public class IndividualClientView extends Div {
  ...
  private void createGrid(ClientService service, Specification<Client> specification) {
    ...
    GridUtils.addColumn(
        grid,
        client -> client.getCedula() != null ? client.getCedula() : client.getPassport(),
        "Identificación",
        "identification");
  ...
  private Predicate createSearchPredicate(Root<Client> root, CriteriaBuilder builder) {
    return predicateForTextField(
        root,
        builder,
        new String[] {"firstName", "lastName", "cedula", "passport", "email"},
  ...
  private Div createTitle() {
    ...
    Icon icon =
        InfoIcon.INFO_CIRCLE.create("Gestionar clientes individuales mediante cédula o pasaporte.");
