...
public class ProductForm extends Dialog {
    ...
    public ProductForm(ProductService productService, SupplierService supplierService,
    ...
    private void createForm() {
        ...
        productInfo.add(name, description, purchasePrice, salesPrice, accountingStock, availableStock, reorderLevel,
                category, supplier, warehouse);
        productInfo.setResponsiveSteps(new FormLayout.ResponsiveStep("0", 1), new FormLayout.ResponsiveStep(
                RESPONSIVE_STEP_WIDTH, 2));
    ...
    private void setupFields(FormLayout productInfo) {
        ...
        warehouse.setItems(warehouseService.getAllWarehouses().stream().map(dto -> {
    ...
    private void createProduct() {
        ProductCreateRequestDto dto = ProductCreateRequestDto.builder().name(name.getValue()).description(description
                .getValue()).purchasePrice(BigDecimal.valueOf(purchasePrice.getValue())).salesPrice(BigDecimal.valueOf(
                        salesPrice.getValue())).accountingStock(accountingStock.getValue().intValue()).availableStock(
                                availableStock.getValue().intValue()).reorderLevel(reorderLevel.getValue().intValue())
                .supplierId(supplier.getValue().getId()).category(category.getValue()).warehouseId(warehouse.getValue()
                        .getId()).build();
    ...
    private void updateProduct() {
        ProductUpdateRequestDto dto = ProductUpdateRequestDto.builder().name(name.getValue()).description(description
                .getValue()).purchasePrice(BigDecimal.valueOf(purchasePrice.getValue())).salesPrice(BigDecimal.valueOf(
                        salesPrice.getValue())).accountingStock(accountingStock.getValue().intValue()).availableStock(
                                availableStock.getValue().intValue()).reorderLevel(reorderLevel.getValue().intValue())
                .supplierId(supplier.getValue().getId()).category(category.getValue()).warehouseId(warehouse.getValue()
    ...
    private void populateForm(Product product) {
        ...
        purchasePrice.setValue(product.getPurchasePrice() != null ? product.getPurchasePrice().doubleValue() : 0.0);
        salesPrice.setValue(product.getSalesPrice() != null ? product.getSalesPrice().doubleValue() : 0.0);
