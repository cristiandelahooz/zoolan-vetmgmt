...
public class ConsultationDetailsSidebar extends Div {
    ...
    public ConsultationDetailsSidebar(InvoiceService invoiceService, ConsultationsForm consultationsForm) {
        ...
        sidebar.setOnSaveClickListener(e -> {
            if (current == null) { sidebar.close(); return; }
            try {
                ...
                if (inv != null && inv.getStatus() == InvoiceStatus.PENDING) {
                    ...
                    NotificationUtils.error("La factura está " + inv.getStatus() + ". Solo se edita si está PENDING.");
                } else {
    ...
    private Component buildClientAndPet(Consultation c) {
        Client owner = null;
        if (c.getPet() != null) {
            List<Client> owners = (c.getPet().getOwners() != null) ? c.getPet().getOwners() : Collections.emptyList();
            owner = owners.stream().findFirst().orElse(null);
        }

        Span clientName = new Span(owner != null ? owner.getFirstName() + " " + owner.getLastName() : "Sin dueño asignado");
        ...
        HorizontalLayout contact = new HorizontalLayout(phoneIcon, phoneText, new Span("•"), mailIcon, emailText);
        ...
        box.addClassNames(LumoUtility.Display.FLEX, LumoUtility.FlexDirection.COLUMN, LumoUtility.Gap.SMALL);
        return box;
    }

    private Component buildConsultationDetails(Consultation c) {
        // Veterinario
        Icon vetIcon = VaadinIcon.USER.create(); vetIcon.setColor("var(--lumo-secondary-text-color)");
        ...
        // Fecha
        Icon dateIcon = VaadinIcon.CALENDAR.create(); dateIcon.setColor("var(--lumo-secondary-text-color)");
        ...
        Icon notesIcon = VaadinIcon.NOTEBOOK.create(); notesIcon.setColor("var(--lumo-secondary-text-color)");
        HorizontalLayout notesRow = new HorizontalLayout(notesIcon,
                new Span("Notas: " + (c.getNotes() != null && !c.getNotes().isBlank() ? c.getNotes() : "N/A")));

        Icon diagIcon = VaadinIcon.CLIPBOARD_HEART.create(); diagIcon.setColor("var(--lumo-secondary-text-color)");
        HorizontalLayout diagRow = new HorizontalLayout(diagIcon,
                new Span("Diagnóstico: " + (c.getDiagnosis() != null && !c.getDiagnosis().isBlank() ? c.getDiagnosis() : "N/A")));

        Icon treatIcon = VaadinIcon.STETHOSCOPE.create(); treatIcon.setColor("var(--lumo-secondary-text-color)");
        HorizontalLayout treatRow = new HorizontalLayout(treatIcon,
                new Span("Tratamiento: " + (c.getTreatment() != null && !c.getTreatment().isBlank() ? c.getTreatment() : "N/A")));

        Icon rxIcon = VaadinIcon.PILL.create(); rxIcon.setColor("var(--lumo-secondary-text-color)");
        HorizontalLayout rxRow = new HorizontalLayout(rxIcon,
                new Span("Prescripción: " + (c.getPrescription() != null && !c.getPrescription().isBlank() ? c.getPrescription() : "N/A")));
    ...
    private LinesAndTotals buildLines(Invoice inv) {
        ...
        grid.addColumn(r -> toMoney(r.importe())).setHeader("Importe").setTextAlign(ColumnTextAlign.END);
        ...
        if (inv != null) {
            if (inv.getServices() != null) {
                rows.addAll(inv.getServices().stream().map(si -> new LineRow(
                        "Servicio",
            ...
            if (inv.getProducts() != null) {
                rows.addAll(inv.getProducts().stream().map(ip -> new LineRow(
                        "Producto",
    ...
    private record LineRow(String tipo, String nombre, Double cantidad, BigDecimal precio, BigDecimal importe) {}
}
