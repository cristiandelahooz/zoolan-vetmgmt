...
public class ConsultationsForm extends Dialog {
    ...
    private void createForm() {
        ...
        formLayout.add(notes, diagnosis, treatment, prescription, consultationDate, consultationTime,
                selectVeterinarianButton, selectedVeterinarianField, selectPetButton, selectedPetField);
        ...
        List<String> timeSlots = IntStream.rangeClosed(8 * 60, 20 * 60).filter(min -> min % 15 == 0).mapToObj(
                min -> String.format("%02d:%02d", min / 60, min % 60)).collect(Collectors.toList());
        ...
        VerticalLayout content = new VerticalLayout(new H3("Información de la Consulta"), formLayout);
    ...
    private void openVeterinarianDialog() {
        ...
        grid.addColumn(e -> e.getEmployeeRole() != null ? e.getEmployeeRole().name() : "").setHeader("Rol").setKey(
                "role");

        grid.addItemClickListener(event -> {
            selectedVeterinarian = event.getItem();
            veterinarianId.setValue(selectedVeterinarian.getId().toString());
            selectedVeterinarianField.setValue(selectedVeterinarian.getId() + " - " + selectedVeterinarian
                    .getFirstName() + " " + selectedVeterinarian.getLastName());
            dialog.close();
        });
        dialog.add(grid);
        dialog.open();
    ...
    private void openPetDialog() {
        ...
        grid.addItemClickListener(event -> {
            ...
            selectedPetField.setValue(selectedPet.getId() + " - " + selectedPet.getName());
            dialog.close();
        });
        dialog.add(grid);
        dialog.open();
    ...
    private void save(ClickEvent<Button> event) {
        ...
        if (editingConsultation == null) {
            CreateConsultationRequestDto dto = CreateConsultationRequestDto.builder().notes(notes.getValue()).diagnosis(
                    diagnosis.getValue()).treatment(treatment.getValue()).prescription(prescription.getValue())
                    .consultationDate(dateTime).petId(Long.parseLong(petId.getValue())).veterinarianId(Long.parseLong(
                            veterinarianId.getValue())).build();
            ...
            UpdateConsultationRequestDto dto = UpdateConsultationRequestDto.builder().notes(notes.getValue()).diagnosis(
                    diagnosis.getValue()).treatment(treatment.getValue()).prescription(prescription.getValue())
                    .consultationDate(dateTime.toString()).petId(Long.parseLong(petId.getValue())).veterinarianId(Long
                            .parseLong(veterinarianId.getValue())).build();
    ...
    public void openForEdit(Consultation consultation) {
        ...
        veterinarianId.setValue(consultation.getVeterinarian() != null ? consultation.getVeterinarian().getId()
                .toString() : "");
