...
public class GroomingDetailsSidebar extends Div {
    ...
    public void open(GroomingSession s) {
        ...
        Invoice inv = invoiceService.findByGroomingIdWithDetails(s.getId())
    ...
    private Component buildHeaderBadges(GroomingSession s, Invoice inv) {
        Span status = new Span(
                (s != null && Boolean.TRUE.equals(s.getActive())) ? "ACTIVO" : "INACTIVO");
        status.getElement().getThemeList().add("badge pill");
        status.getElement().getThemeList().add(
                (s != null && Boolean.TRUE.equals(s.getActive())) ? "success" : "error");
    ...
    private Component buildSessionDetails(GroomingSession s) {
        ...
        HorizontalLayout notesRow = new HorizontalLayout(notesIcon,
                new Span("Notas: " + (s.getNotes() != null && !s.getNotes().isBlank() ? s.getNotes() : "N/A")));
    ...
    private LinesAndTotals buildLines(Invoice inv) {
        ...
        if (inv != null) {
            if (inv.getServices() != null) {
                rows.addAll(inv.getServices().stream()
                        .map(si -> new LineRow(
                                "Servicio",
            ...
            if (inv.getProducts() != null) {
                rows.addAll(inv.getProducts().stream()
                        .map(ip -> new LineRow(
                                "Producto",
