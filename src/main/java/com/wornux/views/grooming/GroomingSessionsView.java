...
public class GroomingSessionsView extends Div {
    ...
    private void createGrid(GroomingSessionService service, Specification<GroomingSession> specification) {
        GridUtils.configureGrid(grid, specification, service.getRepository());

        GridUtils.addColumn(grid, s -> s.getPet() != null ? s.getPet().getName() : "", "Mascota", "pet");
        GridUtils.addColumn(grid, s -> s.getGroomer() != null ? s.getGroomer().getFirstName() : "", "Groomer", "groomer");
        GridUtils.addColumn(grid, GroomingSession::getGroomingDate, "Fecha de Grooming", "groomingDate");
    ...
    private void refreshAll() {
        List<GroomingSession> sessions = groomingSessionService.findAll(Pageable.unpaged()).getContent();
            ...
            sessions = sessions.stream()
                    .filter(s ->
                            (s.getNotes() != null && s.getNotes().toLowerCase().contains(filter)) ||
                                    (s.getPet() != null && s.getPet().getName() != null && s.getPet().getName().toLowerCase().contains(filter)) ||
                                    (s.getGroomer() != null && (
                                            (s.getGroomer().getFirstName() != null && s.getGroomer().getFirstName().toLowerCase().contains(filter)) ||
                                                    (s.getGroomer().getLastName() != null && s.getGroomer().getLastName().toLowerCase().contains(filter))
                                    ))
                    )
                    .toList();
        }
    ...
    private void showDeleteConfirmationDialog(GroomingSession session) {
        ...
        Span message = new Span("¿Está seguro de que desea eliminar el grooming de la mascota \"" +
