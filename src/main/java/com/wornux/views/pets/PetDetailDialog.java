...
public class PetDetailDialog extends Dialog {
    ...
    public PetDetailDialog(Pet pet, ConsultationService consultationService) {
        ...
        List<Consultation> consultations = Optional.ofNullable(consultationService.findByPetId(pet.getId()))
        ...
        Button viewHistoryBtn = new Button("Ver historial", e -> {
            close();
            UI.getCurrent()
                    .navigate(MedicalHistoryView.class, new RouteParameters(Map.of("petId", pet.getId().toString())));
        });
        viewHistoryBtn.setIcon(VaadinIcon.OPEN_BOOK.create());
        viewHistoryBtn.getStyle().set("flex", "0 0 auto") // no shrink
    ...
    private VerticalLayout buildHeaderCard() {
        ...
        FormLayout kvRow = new FormLayout(kvItem("Tipo", fmtValueUpper(type), "primary"),
                kvItem("Raza", fmtValueUpper(breed), "contrast"), kvItem("Género", fmtValueUpper(gender), "success"),
                kvItem("Dueño", fmtValueUpper(owner), "contrast"));
        kvRow.setWidthFull();
        kvRow.setResponsiveSteps(new ResponsiveStep("0", 1), new ResponsiveStep("480px", 2),
    ...
    private VerticalLayout buildClinicalSnapshot(List<Consultation> consultations) {
        ...
        panel.getStyle().set("border", "1px solid var(--lumo-contrast-10pct)")
        ...
        String lastTreatment = consultations.stream().map(Consultation::getTreatment)
                .filter(s -> s != null && !s.isBlank()).findFirst().orElse("—");

        String lastPrescription = consultations.stream().map(Consultation::getPrescription)
                .filter(s -> s != null && !s.isBlank()).findFirst().orElse("—");
    ...
    private Optional<String> detectAllergyFrom(List<Consultation> consultations) {
        ...
        return consultations.stream().flatMap(c -> Arrays.stream(
                new String[] { nullSafeLower(c.getDiagnosis()), nullSafeLower(c.getNotes()),
                        nullSafeLower(c.getPrescription()) })).filter(Objects::nonNull).filter(txt -> {
            for (String k : keys)
                if (txt.contains(k))
                    return true;
            return false;
    ...
    private List<String> topRecentDiagnoses(List<Consultation> consultations, int limit) {
        return consultations.stream().map(Consultation::getDiagnosis).filter(s -> s != null && !s.isBlank())
                .map(String::trim).map(this::normalizeDiagnosis).filter(s -> !s.isBlank()).distinct().limit(limit)
    ...
    private VerticalLayout buildLastConsultations(List<Consultation> consultations) {
        ...
        last3.forEach(c -> {
            ...
            String dx = (c.getDiagnosis() != null && !c.getDiagnosis()
                    .isBlank()) ? c.getDiagnosis() : "Sin diagnóstico";
    ...
    private String fmtValueUpper(String s) {
        return s == null ? "—" : s.toUpperCase(Locale.ROOT);
    }

    private String fmtValueLower(String s) {
