...
public class PetDetailDialog extends Dialog {
  ...
  public PetDetailDialog(Pet pet, ConsultationService consultationService) {
    ...
    List<Consultation> consultations =
        Optional.ofNullable(consultationService.findByPetId(pet.getId()))
            .orElseGet(List::of)
            .stream()
  ...
  private VerticalLayout buildHeaderCard() {
    ...
    kvRow.setResponsiveSteps(
        new ResponsiveStep("0", 1), new ResponsiveStep("480px", 2), new ResponsiveStep("800px", 4));
  ...
  private VerticalLayout buildLastConsultations(List<Consultation> consultations) {
    ...
    last3.forEach(
        c -> {
          ...
          String date =
              c.getConsultationDate() != null ? DATE_FMT.format(c.getConsultationDate()) : "—";
          String dx =
              (c.getDiagnosis() != null && !c.getDiagnosis().isBlank())
                  ? c.getDiagnosis()
                  : "Sin diagnóstico";
          String tr =
              (c.getTreatment() != null && !c.getTreatment().isBlank()) ? c.getTreatment() : "—";
