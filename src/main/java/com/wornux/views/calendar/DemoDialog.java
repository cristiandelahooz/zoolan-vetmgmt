...
public class DemoDialog extends Dialog {
  ...
  @Setter private SerializableConsumer<Entry> onSaveConsumer;
  @Setter private SerializableConsumer<Entry> onDeleteConsumer;
  ...
  public DemoDialog(Entry entry, boolean newInstance) {
    ...
    // tmp entry is a copy. we will use its start and end to represent either the start/end or the
    // recurring start/end
    ...
    fieldAllDay.addValueChangeListener(
        event -> {
    ...
    fieldStart.addValueChangeListener(
        event -> {
    ...
    fieldEnd.addValueChangeListener(
        event -> {
          if (event.isFromClient()) {
            resetPeriodOnAllDayChange = false;
          }
