...
public abstract class AbstractCalendarView extends VerticalLayout {
  ...
  /**
   * Creates the plain full calendar instance with all initial options. The given default initial
   * options are created by {@link #createDefaultInitialOptions()} beforehand.
   *
   * <p>The calender is automatically embedded afterwards and connected with the toolbar (if one is
   * created, which is the default). Also all event listeners will be initialized with a default
   * callback method.
   *
  ...
  /**
   * Called by the calendar's entry drop listener (i. e. an entry has been dragged around / moved by
   * the user). Applies the changes to the entry and calls {@link #onEntryChanged(Entry)} by
   * default.
   *
  ...
  /**
   * Called by the calendar's entry resize listener. Applies the changes to the entry and calls
  ...
  /**
   * Called by the calendar's dates rendered listener. Noop by default. Please note, that there is a
   * separate dates rendered listener taking care of updating the toolbar.
   *
  ...
  /**
   * Called by the calendar's browser timezone obtained listener. Noop by default. Please note, that
   * the full calendar builder registers also a listener, when the {@link
   * FullCalendarBuilder#withAutoBrowserTimezone()} option is used.
   *
   ...
   * parameters are also enabled by default. Either update the given variable or create a new one,
   * if necessary. Return null for no toolbar at all.
   *
   * @param toolbarBuilder toolbar builder
   * @return modified or new instance
   */
  protected CalendarViewToolbarBuilder initToolbarBuilder(
      CalendarViewToolbarBuilder toolbarBuilder) {
    return toolbarBuilder;
  }

  /**
   * Creates the toolbar. The parameter might be null depending on a custom implementation of {@link
   * #initToolbarBuilder(CalendarViewToolbarBuilder)}. Return null if no toolbar shall be available.
   *
  ...
  /**
   * Called by the toolbar, when one of the "Create sample entries" button has been pressed to
   * simulate the creation of new data. Might be called by any other source, too.
   *
   * <p>Intended to update the used backend. By default it will check, if the used entry provider is
   * eager in memory and in that case automatically update the entry provider (to prevent
   * unnecessary code duplication when the default entry provider is used).
   *
   * @param entries entries to add
   */
  protected void onEntriesCreated(Collection<Entry> entries) {
    // The eager in memory provider provider provides API to modify its internal cache and takes
    // care of pushing
  ...
  /**
   * Called by the toolbar, when the "Remove entries" button has been pressed to simulate the
   * removal of entries. Might be called by any other source, too.
   *
   * <p>Intended to update the used backend. By default it will check, if the used entry provider is
   * eager in memory and in that case automatically update the entry provider (to prevent
   * unnecessary code duplication when the default entry provider is used).
  ...
  /**
   * Called, when one of the sample entries have been modified, e. g. by an event. Might be called
   * by any other source, too.
   *
   * <p>Intended to update the used backend. By default it will check, if the used entry provider is
   * eager in memory and in that case automatically update the entry provider (to prevent
   * unnecessary code duplication when the default entry provider is used).
  ...
  /**
   * Returns the entry provider set to the calendar. Will be available after {@link
   * #createCalendar(JsonObject)} has been called.
   *
