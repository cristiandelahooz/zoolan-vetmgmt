...
public class CalendarViewToolbar extends MenuBar {
  ...
  private CalendarViewToolbar(
      ...
      boolean allowAddingRandomItemsInitially,
      Consumer<Collection<Entry>> onSamplesCreated,
      Consumer<Collection<Entry>> onSamplesRemoved,
      List<org.vaadin.stefan.fullcalendar.CalendarView> customViews) {
                      ...
                      Executors.newSingleThreadExecutor()
                          .execute(
                              () -> {
                                ...
                                entry.setRecurringEndDate(
                                    entry.getRecurringStartDate().plusYears(1));
                                entry.setRecurringEndTime(
                                    entry.getRecurringStartTime().plusMinutes(60));
                                ...
                                ui.access(
                                    () -> {
                                      onSamplesCreated.accept(Collections.singletonList(entry));
                                      Notification.show(
                                          "Added a recurring entry for this year, each monday at 10:00 UTC");
                                    });
                              });
                    });
              });
                    ...
                    ui -> {
                      Executors.newSingleThreadExecutor()
                          .execute(
                              () -> {
                                List<Entry> entries =
                                    EntryService.createRandomInstance().getEntries();
                                ui.access(
                                    () -> {
                                      onSamplesCreated.accept(entries);
                                      Notification.show(
                                          "Added " + entries.size() + " random entries");
                                    });
                              });
                    });
              });
  ...
  private SubMenu initGeneralSettings() {
    ...
    themeSelector.addValueChangeListener(
        event -> {
    ...
    localeSelector.addValueChangeListener(
        event -> {
    ...
    timezoneSelector.addValueChangeListener(
        event -> {
    ...
    enablePrefetching.addValueChangeListener(
        event -> calendar.setPrefetchEnabled(event.getValue()));
    ...
    multiMonthColumns.addThemeVariants(
        TextFieldVariant.LUMO_SMALL, TextFieldVariant.LUMO_ALIGN_CENTER);
    multiMonthColumns.addValueChangeListener(
        event -> calendar.setOption(FullCalendar.Option.MULTI_MONTH_MAX_COLUMNS, event.getValue()));
    ...
    validRangeStart.addValueChangeListener(
        event -> {
    ...
    validRangeEnd.addValueChangeListener(
        event -> {
          calendar.setValidRange(validRangeStart.getValue(), validRangeEnd.getValue());
  ...
  private void initViewSelector() {
    List<org.vaadin.stefan.fullcalendar.CalendarView> calendarViews;
    ...
    calendarViews.sort(Comparator.comparing(org.vaadin.stefan.fullcalendar.CalendarView::getName));
  ...
  private String getViewName(org.vaadin.stefan.fullcalendar.CalendarView view) {
    String name = null /*customViewNames.get(view)*/;
    if (name == null) {
      name =
          StringUtils.capitalize(
              String.join(
                  " ", StringUtils.splitByCharacterTypeCamelCase(view.getClientSideValue())));
    }

    return name;
  }

  public void updateSelectedView(org.vaadin.stefan.fullcalendar.CalendarView view) {
  ...
  void updateIntervalLabel(
    ...
    if (view instanceof HasIntervalLabel intervalLabelView) {
      ...
      String pattern =
          view != null && view.getDateTimeFormatPattern() != null
              ? view.getDateTimeFormatPattern()
              : "MMMM yyyy";
