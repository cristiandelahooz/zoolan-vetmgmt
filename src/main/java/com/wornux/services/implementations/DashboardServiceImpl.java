...
List<Object[]> results = invoiceRepository.findMonthlyRevenue(
    ...
    if (!revenueData.isEmpty()) {
      double avgRevenue = revenueData.stream()
          .mapToDouble(RevenueDataDto::getRevenue)
          .average()
          .orElse(0.0);
  ...
  public List<ChartDataDto> getTopServicesAnalysis() {
    ...
    List<Object[]> servicesData = invoiceRepository.findTopServicesByRevenue(startDateInstant, topTen);

    return servicesData.stream()
        .map(data -> new ChartDataDto((String) data[0], ((Number) data[1]).doubleValue(), null, null))
  ...
  public List<StockAlertDto> getStockHealthAnalysis() {
    ...
    return results.stream()
        .map(row -> {
          ...
          return new StockAlertDto(productId, productName, availableStock,
              5, category, alertLevel); // Using 5 as minimum stock
        })
  ...
  public List<ChartDataDto> getConsultationTrends() {
    ...
    List<Object[]> results = consultationRepository.findDailyConsultationCounts(
        threeMonthsAgo, now);
  ...
  public Map<String, Object> getClientRetentionMetrics() {
    ...
    Long newClients = clientRepository.countNewClientsByPeriod(threeMonthsAgo.atZone(ZoneId.systemDefault()).toInstant(), LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant());
    Long returningClients = clientRepository.countReturningClientsByPeriod(threeMonthsAgo.atZone(ZoneId.systemDefault()).toInstant(), LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant());
  ...
  public List<ChartDataDto> getEmployeeUtilizationData() {
    ...
    List<Object[]> results = consultationRepository.findEmployeeUtilizationByHour(
        fourWeeksAgo, now);
