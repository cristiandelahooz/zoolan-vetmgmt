...
public class ConsultationServiceImpl extends ListRepositoryService<Consultation, Long, ConsultationRepository>
    ...
    public Consultation create(CreateConsultationRequestDto createDTO) {
        ...
        Pet pet = petRepository.findById(createDTO.getPetId()).orElseThrow(() -> new PetNotFoundException(createDTO
                .getPetId()));

        Employee veterinarian = employeeRepository.findById(createDTO.getVeterinarianId()).orElseThrow(
                () -> new EmployeeNotFoundException(createDTO.getVeterinarianId()));
    ...
    private void updateMedicalHistoryFromConsultation(MedicalHistory medicalHistory, Consultation consultation) {
        ...
        String consultationInfo = String.format(
                "\n--- Consulta del %s ---\nDiagnóstico: %s\nTratamiento: %s\nPrescripción: %s\nNotas: %s\n",
                consultation.getConsultationDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")), consultation
                        .getDiagnosis() != null ? consultation.getDiagnosis() : "No especificado", consultation
                                .getTreatment() != null ? consultation.getTreatment() : "No especificado", consultation
                                        .getPrescription() != null ? consultation.getPrescription() : "No especificado",
    ...
    public Consultation update(Long id, UpdateConsultationRequestDto updateDTO) {
        ...
        Pet pet = petService.getPetById(updateDTO.getPetId()).orElseThrow(() -> new EntityNotFoundException(
    ...
    public Consultation findById(Long id) {
        ...
        return consultationRepository.findById(id).orElseThrow(() -> new EntityNotFoundException(
