...
public class PetServiceImpl extends ListRepositoryService<Pet, Long, PetRepository>
  ...
  public Pet updatePet(Long id, @Valid PetUpdateRequestDto dto) {
    ...
    if (dto.getOwnerIds() != null) {
      var owners = clientService.findAllActiveByIds(dto.getOwnerIds()); // o findAllByIds
  ...
  public Pet mergePets(Long keepPetId, Long removePetId) {
    ...
    Pet keepPet =
        petRepository.findById(keepPetId).orElseThrow(() -> new PetNotFoundException(keepPetId));
    Pet removePet =
        petRepository
            .findById(removePetId)
    ...
    if (!keepPet.getName().equalsIgnoreCase(removePet.getName())
      ...
      throw new IllegalArgumentException(
          "Las mascotas no parecen ser la misma (nombre o tipo diferentes)");
    }

    Set<Long> existingOwnerIds =
        keepPet.getOwners().stream().map(Client::getId).collect(Collectors.toSet());

    List<Client> newOwners =
        removePet.getOwners().stream()
    ...
    log.info(
        "Merged pets: {} total owners now associated with pet ID {}",
        keepPet.getOwners().size(),
        keepPetId);
