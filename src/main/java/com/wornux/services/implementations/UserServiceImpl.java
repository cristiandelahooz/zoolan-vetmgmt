...
public class UserServiceImpl implements UserService {
    ...
    public User createUser(String username, String password, @Nullable String email, String firstName, String lastName,
        ...
        var user = User.builder().username(username).password(passwordEncoder.encode(password)).email(email).firstName(
                firstName).lastName(lastName).phoneNumber(phoneNumber).birthDate(birthDate).gender(gender).nationality(
                        nationality).province(province).municipality(municipality).sector(sector).streetAddress(
                                streetAddress).systemRole(systemRole).active(true).createdAt(LocalDateTime.now(clock))
    ...
    public User updateUser(Long userId, User updateData) {
        ...
        if (!currentUser.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN")) && !user
                .getUsername().equals(currentUser.getName())) {
    ...
    public Optional<User> getUserById(Long userId) {
        ...
        if (user.isPresent() && !currentUser.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals(
                "ROLE_ADMIN")) && !user.get().getUsername().equals(currentUser.getName())) {
    ...
    public List<User> searchUsers(@Nullable String searchTerm, @Nullable SystemRole systemRole,
        ...
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            spec = spec.and((root, query, cb) -> cb.or(cb.like(cb.lower(root.get("username")), "%" + searchTerm
                    .toLowerCase() + "%"), cb.like(cb.lower(root.get("email")), "%" + searchTerm.toLowerCase() + "%"),
                    cb.like(cb.lower(root.get("firstName")), "%" + searchTerm.toLowerCase() + "%"), cb.like(cb.lower(
                            root.get("lastName")), "%" + searchTerm.toLowerCase() + "%"), cb.like(cb.lower(root.get(
                                    "province")), "%" + searchTerm.toLowerCase() + "%"), cb.like(cb.lower(root.get(
                                            "municipality")), "%" + searchTerm.toLowerCase() + "%"), cb.like(cb.lower(
                                                    root.get("sector")), "%" + searchTerm.toLowerCase() + "%")));
        }
