...
public class ClientServiceImpl extends ListRepositoryService<Client, Long, ClientRepository>
    ...
    private void validateUniqueIdentificationForUpdate(Long clientId, String cedula, String passport, String rnc) {
        // Check for null values before calling trim()
        if (cedula != null && !cedula.trim().isEmpty()) {
            Optional<Client> existingClientWithCedula = clientRepository.findByCedulaAndIdNot(cedula, clientId);
        ...
        if (passport != null && !passport.trim().isEmpty()) {
            Optional<Client> existingClientWithPassport = clientRepository.findByPassportAndIdNot(passport, clientId);
