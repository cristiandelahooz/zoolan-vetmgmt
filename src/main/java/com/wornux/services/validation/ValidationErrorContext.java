...
/**
 * Context configuration for validation error handling.
 * Contains settings that control how validation errors are displayed to the user.
 * 
 * <p>This class uses the builder pattern to provide flexible configuration
 * while maintaining sensible defaults for common use cases.</p>
 * 
...
public class ValidationErrorContext {
   ...
   */
  @Builder.Default
  Notification.Position position = Notification.Position.TOP_CENTER;
   ...
   */
  @Builder.Default
  NotificationVariant themeVariant = NotificationVariant.LUMO_ERROR;
  
  /**
   * Whether to show notifications with staggered timing to prevent overlap.
   */
  @Builder.Default
  boolean staggeredTiming = false;
   ...
   */
  @Builder.Default
  boolean includeTechnicalDetails = false;
  ...
  /**
   * Creates a context optimized for dialog form validation.
   * Uses individual notifications with staggered timing for better UX.
   */
  ...
  /**
   * Creates a context optimized for showing individual validation errors without delay.
   * Ideal for cases where immediate feedback is needed.
   */
