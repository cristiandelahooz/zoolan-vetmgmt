...
public final class GridUtils {
  ...
  public static <T> void configureGrid(
      Grid<T> grid, CallbackDataProvider.FetchCallback<T, Void> fetchCallback) {
    grid.setItems(fetchCallback);
  }

  public static <T> void configureGrid(
      Grid<T> grid, Specification<T> specification, JpaSpecificationExecutor<T> repository) {
    grid.setItems(
        query ->
            repository
                .findAll(
  ...
  public static <T> Grid.Column<T> addColumn(
      Grid<T> grid, Renderer<T> renderer, String header, String... sortProperties) {
  ...
  public static <T, V extends Component> Grid.Column<T> addComponentColumn(
      Grid<T> grid,
      ValueProvider<T, V> componentProvider,
      String header,
      String... sortProperties) {
    Grid.Column<T> tColumn =
        grid.addColumn(new ComponentRenderer<>(componentProvider))
            .setAutoWidth(true)
            .setHeader(header);
  ...
  public static <T> Grid.Column<T> addColumn(
      Grid<T> grid,
      ValueProvider<T, ?> componentProvider,
      String header,
      String... sortProperties) {
    return grid.addColumn(componentProvider)
        .setAutoWidth(true)
        .setHeader(header)
        .setSortable(sortProperties != null && sortProperties.length > 0)
        .setSortProperty(sortProperties);
  }
}
