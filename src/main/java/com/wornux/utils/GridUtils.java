...
public final class GridUtils {
    ...
    public static <T> Grid.Column<T> addColumn(Grid<T> grid, ValueProvider<T, ?> componentProvider, String header,
            String... sortProperties) {
        return grid.addColumn(componentProvider).setAutoWidth(true).setHeader(header)
                .setSortable(sortProperties != null && sortProperties.length > 0).setSortProperty(sortProperties);
    }
