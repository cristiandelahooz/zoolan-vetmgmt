...
/**
 * Utility class that provides static access to validation error handling functionality.
 * Acts as a bridge between static utility usage and the Spring-managed validation services.
 * 
 * <p>This utility maintains backward compatibility while providing access to the new
 * centralized validation error handling system.</p>
 * 
...
public class ValidationNotificationUtils implements ApplicationContextAware {
   ...
   */
  public static void handleValidationErrors(ConstraintViolationException exception, ValidationErrorContext context) {
  ...
  public static void handleCalendarValidationErrors(ConstraintViolationException exception) {
    getValidationErrorHandler().handleValidationErrors(
        exception, 
        ValidationErrorContext.forCalendarOperations()
    );
  }
  ...
  public static void handleDialogValidationErrors(ConstraintViolationException exception) {
    getValidationErrorHandler().handleValidationErrors(
        exception, 
        ValidationErrorContext.forDialogForms()
    );
  }
  ...
  public static void handleFormValidationErrors(ConstraintViolationException exception) {
    getValidationErrorHandler().handleValidationErrors(
        exception, 
        ValidationErrorContext.forGeneralForms()
    );
  }

  /**
   * Handles validation errors with immediate individual notifications.
   * Ideal for cases requiring instant feedback without staggered timing.
   * 
   * @param exception the constraint violation exception to handle
   */
  public static void handleIndividualValidationErrors(ConstraintViolationException exception) {
    getValidationErrorHandler().handleValidationErrors(
        exception, 
        ValidationErrorContext.forIndividualErrorsImmediate()
    );
  }
  ...
  private static ValidationErrorHandler getValidationErrorHandler() {
    if (applicationContext == null) {
      throw new IllegalStateException("ApplicationContext not initialized. Ensure ValidationNotificationUtils is properly configured as a Spring bean.");
    }
